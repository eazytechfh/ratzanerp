...
import { X, Plus, Trash2, Bug } from 'lucide-react'
import { useTiposPraga, addTipoPraga, updateTipoPraga, removeTipoPraga } from '../data/tipoPragaStore'

interface Props {
  onClose: () => void
}

export default function TiposPragaModal({ onClose }: Props) {
  const tipos = useTiposPraga()
  const [novoNome, setNovoNome] = useState('')

  function handleAdd(e: React.FormEvent) {
    e.preventDefault()
    if (!novoNome.trim()) return
    addTipoPraga({ id: `tpp-${Date.now()}`, nome: novoNome.trim() })
    setNovoNome('')
  }
      ...
      <div className="relative bg-white rounded-2xl shadow-soft w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
          <h2 className="text-lg font-bold text-ink-900 flex items-center gap-2">
            <Bug size={18} className="text-brand-600" />
            Tipos de praga
          </h2>
          ...
          <div className="space-y-2">
            {tipos.map((tipo) => (
              <div key={tipo.id} className="flex items-center gap-3 rounded-lg border border-slate-200 px-3 py-2.5">
                <input
                  value={tipo.nome}
                  onChange={(e) => updateTipoPraga(tipo.id, { nome: e.target.value })}
                  className="flex-1 text-sm text-ink-900 outline-none bg-transparent"
                />
                <button
                  onClick={() => removeTipoPraga(tipo.id)}
            ...
            {tipos.length === 0 && (
              <p className="text-sm text-slate-400 text-center py-4">Nenhum tipo de praga cadastrado.</p>
            )}
          </div>
        </div>
      </div>
