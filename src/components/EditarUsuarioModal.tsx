...
import type { UserRole, Operador, Perfil } from '../types'
import { USER_ROLES, USER_ROLE_LABELS } from '../types'
import { editarUsuario } from '../data/usuarioClient'
...
export default function EditarUsuarioModal({ usuario, operadores, podeAlterarPapel, onClose }: Props) {
  const [nome, setNome] = useState(usuario.nome)
  const [role, setRole] = useState<UserRole>(usuario.role)
  const [operadorId, setOperadorId] = useState(usuario.operadorId ?? '')
      ...
      <div className="relative bg-white rounded-2xl shadow-soft w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input value={usuario.email} disabled className="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm bg-slate-50 text-slate-400" />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm outline-none focus:border-brand-500 bg-white disabled:bg-slate-50 disabled:text-slate-400"
            >
              {USER_ROLES.map((r) => (
                <option key={r} value={r}>{USER_ROLE_LABELS[r]}</option>
              ))}
            </select>
            {!podeAlterarPapel && <p className="text-xs text-slate-400 mt-1">Você só pode editar dados de usuários com papel Operador.</p>}
          </div>
