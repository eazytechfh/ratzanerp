...
export default function FluxoCaixaTab() {
  ...
  function handleExportar() {
    const linhas = [
      ['Data', 'Tipo', 'Descrição', 'Contraparte', 'Status', 'Valor'],
    ...
    a.href = url
    a.download = `fluxo-de-caixa-${new Date().toISOString().slice(0, 10)}.csv`
    a.click()
          ...
          <div className="flex items-center gap-2">
            <div className="flex gap-1 bg-slate-100 rounded-lg p-1">
              {([
                ...
                { key: 'entrada', label: 'Receita' },
                { key: 'saida', label: 'Despesa' },
              ] as const).map((f) => (
                <button
                  key={f.key}
                  onClick={() => setFiltroTipo(f.key)}
                  className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${
                    filtroTipo === f.key ? 'bg-white text-ink-900 shadow-sm' : 'text-slate-500 hover:text-ink-900'
                  }`}
                >
                  {f.label}
                </button>
